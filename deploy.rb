...
case input.upcase
  when "1"
    server = 'ec2.us-west-2.compute.amazonaws.com' # this is development instance
    server_uri = 'ec2.us-west-2.compute.amazonaws.com'
    env = 'development' # the dev server is in test env
  when "2"
    server = 'ec2.compute.amazonaws.com'
    server_uri = 'ec2.compute.amazonaws.com'
    env = 'staging'  # the dev server is in test env
  when "3"
    server = 'abc.com' # this is staging instance
    server_uri = 'abc.com'
    env = 'production' # run the server in production mode 
...
set :domain, server
set :deploy_to, '/var/app/swapnil'
set :repository, 'git@github.com:poaster/mygit.git'
set :branch, branch
set :rails_env, env
...
set :shared_paths, ['config/database.yml', 'config/aws.yml', 'log','config/settings/staging.yml','config/settings/production.yml']
